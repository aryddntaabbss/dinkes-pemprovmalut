    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h1 class="text-2xl font-semibold mb-6">Edit Halaman Informasi</h1>

                <form method="POST" action="{{ route('informasi.update', $informasi->slug) }}"
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $informasi->name) }}"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                            required placeholder="Masukkan nama halaman" oninput="generateSlug()" @if ($informasi->slug
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="slug" name="slug"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                            value="{{ old('slug', $informasi->slug) }}" required readonly>
                        @error('slug')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="content" name="content"
                            class="form-control mt-1 w-full border border-gray-300 rounded-md shadow-sm">{{ old('content', $informasi->content) }}</textarea>
                        @error('content')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
