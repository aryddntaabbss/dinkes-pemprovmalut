<x-app-layout>
                            ...
                            <tr class="border-t hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-900">
                                    ...
                                    <form id="delete-form-{{ $item->id }}"
                                        action="{{ route('dashboard.doc-lainx.destroy', $item->id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
    ...
    <div id="add-file-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg shadow-lg max-w-md w-full p-6">
            ...
            <form action="{{ route('dashboard.doc-lainx.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <!-- Input Nama -->
                <div class="mb-4">
